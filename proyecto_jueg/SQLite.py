...
def mostrar_datos(screen):
    ...
    while running:
        ...
        draw_text("SCORE",screen,(600,100),60,"white")
        draw_text("PLAYER",screen,(300,100),60,"white")
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                    running = False
        for row in rows:
            draw_text(str(row[2]),screen,(600,y),40,"red")
            draw_text(str(row[1]),screen,(300,y),40,"blue")
            y += 50
        y = 150
